...
def page_pixels_from_points(rect, dpi):
    # convert page size in points (1/72") to pixels at dpi
...
def images_similar(img_bytes_a, img_bytes_b, resize=(400, 400), threshold=0.92):
    # confronto rapido: ridimensiona e calcola similarità basata su MSE -> 1 - normalized_mse
    a = Image.open(io.BytesIO(img_bytes_a)).convert("RGB")
    b = Image.open(io.BytesIO(img_bytes_b)).convert("RGB")
    a = a.resize(resize, Image.LANCZOS)
    b = b.resize(resize, Image.LANCZOS)
    # calcola MSE
...
def extract_best_image_or_render(page, doc, dpi_render, prefer_full_image, fallback_render):
    ...
    if prefer_full_image and candidates:
        ...
        similar, sim_score = images_similar(img_bytes, rendered_bytes, resize=(300,300), threshold=0.90)
        if similar:
...
def pdf_to_cbz(pdf_path, dpi=300, extract_images=False, prefer_full_image=False, fallback_render=True, compression=zipfile.ZIP_STORED):
    ...
    with zipfile.ZipFile(cbz_name, "w", compression=compression) as cbz:
        for page_number in range(total_pages):
            ...
            if extract_images:
                img_bytes, ext, source = extract_best_image_or_render(page, doc, dpi, prefer_full_image, fallback_render)
...
def main():
    ...
    parser.add_argument("--dpi", type=int, default=300, help="DPI per il rendering delle pagine (default 300)")
    parser.add_argument("-e", "--extract-images", action="store_true", help="Prova a estrarre immagini incorporate quando possibile")
    parser.add_argument("--prefer-full-image", action="store_true", help="Accetta solo immagini che coprono gran parte della pagina")
    parser.add_argument("--no-fallback", dest="fallback", action="store_false", help="Se usato con -e e --prefer-full-image, non eseguire il rendering se non trovi immagini full-slide")
